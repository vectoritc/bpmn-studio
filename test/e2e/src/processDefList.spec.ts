...
fdescribe('processDefList', () => {
  ...
  const processModel: string = processDefListPage.processModel;

  browser.driver.manage().deleteAllCookies();

  beforeEach(() => {
    browser.get(aureliaUrl + '/processdef');
    browser.driver.wait(() => {
  ...
  it('should display process definitions', () => {
    ...
    processDefinitionListItems.count().then((count: number) => {
      expect(count).toBeGreaterThan(0);
    });

    // Search for the process diagram created in this test
    const processDefinitionListItemIDs: ElementArrayFinder = processDefListPage.processDefinitionListItemIDs;
    processDefinitionListItemIDs.count().then((result: number) => {
      expect(result).toBe(1);
    });
  });

  it('should be possible to open a process diagram', () => {
    const processModellDiagram: ElementFinder = processDefListPage.processModellDiagram;

    // Click on a process definition
    processModellDiagram.click().then(() => {
      browser.getCurrentUrl().then((url: string) => {

        // Check if diagram showed up
        expect(url).toContain('processdef/' + processModel + '/detail');
      });
    });
  });
